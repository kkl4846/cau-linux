...
static ssize_t pxt4_file_write_iter_internal(struct kiocb *iocb, struct iov_iter *from)
{
	struct inode *inode = file_inode(iocb->ki_filp);
...
static unsigned long get_thread_idx(void *elem)
{
	...
	return (unsigned long) current->cpu;
}
...
static void print_zone_dm(unsigned long pid, const char *name, unsigned long long count, int percentage)
{
	printk("thread %s: pid %ld called pxt4_file_write_iter() %lld times (%d%%)\n", name, pid, count, percentage);
}
